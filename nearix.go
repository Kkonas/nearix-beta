...
func initCheck(confStruct *Config, langStruct *LangConfig) {
	...
	if confStruct.Token == "" {
		...
		fmt.Println("Please enter your discord email and password to get your token.")
		fmt.Println("Type it like this: user@email.com password123")
		response := readStdin()
		split := strings.Split(response, " ")
		getToken, _ := discordgo.New(split[0], split[1])
		token := getToken.Token
		defer getToken.Close()
		confStruct.Token = token
		updateConfigYaml()
	}
}
	...
	if message.Author.ID == conf.Constants.PokeCordID {
		if message.Embeds != nil {
			...
			for _, embed := range embeds {
				if embed.Image != nil && (embed.Author == nil || (embed.Author.Name != "Professor Oak" && !strings.Contains(embed.Author.Name, "VS"))) {
					...
					for _, guild := range conf.Session.Guilds {
						if guild.ID == message.GuildID && guild.Enabled {
							...
							shouldCatch.delay = RandomInt(guild.Delay)
							shouldCatch.prefix = guild.Prefix
						}
					}
					if shouldCatch.should {
						pokename := receive(embed.Image.URL)
						if pokename != "" {
							time.Sleep(time.Duration(shouldCatch.delay) * time.Second)
							session.ChannelMessageSend(message.ChannelID, shouldCatch.prefix+"catch "+pokename)
						}
					}
				}
			}
	...
	if len(conf.Session.Guilds) == 0 {
		for index, guild := range client.State.Guilds {
			...
			for _, channel := range guild.Channels {
				...
				appendChannel := Channel{channel.ID, channel.Name, 20, false}
...
func init() {
	...
	spam = &SpamInstance{Channel: totalChannels, Sent: 0}
}
...
func main() {
	...
	<-sc
	fmt.Println("\nSent Messages in this session: " + strconv.Itoa(spam.Sent))
	client.Close()
}
